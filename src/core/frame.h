...
struct Frame {
  ...
  static Vector3f refract(const Vector3f &wi, float cosThetaT, float eta) {
  ...
  static float sinTheta(const Vector3f &v) { return std::sqrt(sinTheta2(v)); }
  ...
  static float sinTheta2(const Vector3f &v) {
    return std::max(0.f, 1.0f - v.z() * v.z());
  }
