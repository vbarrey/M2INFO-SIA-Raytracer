...
static const std::string kWarpTypeNames[WarpTypeCount] = {
    "square",         "triangle",           "disk",
    "uniform_sphere", "uniform_hemisphere", "cosine_hemisphere",
    "beckmann",       "phong_brdf",         "microfacet_brdf"};
...
struct WarpTest {
  ...
  WarpType warpType;
  float parameterValue, parameter2Value;
  BSDF *bsdf;
  ...
  WarpTest(WarpType warpType_, float parameterValue_, float parameter2Value_,
           BSDF *bsdf_ = nullptr,
           BSDFQueryRecord bRec_ = BSDFQueryRecord(::Vector3f()),
           int xres_ = kDefaultXres, int yres_ = kDefaultYres)
      : warpType(warpType_), parameterValue(parameterValue_),
        parameter2Value(parameter2Value_), bsdf(bsdf_), bRec(bRec_),
  ...
  std::pair<bool, std::string> run() {
    ...
    auto integrand = [&](double y, double x) -> double {
      if (warpType == Square) {
        ...
        ::Vector3f v((float)(sinTheta * cosPhi), (float)(sinTheta * sinPhi),
                     (float)y);
        ...
        else if (warpType == PhongBRDF || warpType == MicrofacetBRDF) {
    ...
    if (warpType == Square)
      scale *= 1;
    else if (warpType == Disk || warpType == Triangle)
      scale *= 4;
    else
      scale *= 4 * M_PI;
  ...
  void generatePoints(int &pointCount, PointType pointType,
    ...
    PropertyList list;
    list.setInteger("sampleCount", pointCount);
    list.setInteger("xPixelSamples", sqrtVal);
    list.setInteger("yPixelSamples", sqrtVal);
    list.setInteger("dimension", 1);
    Sampler *sampler;
    switch (pointType) {
    case Independent:
      sampler = static_cast<Sampler *>(
          ObjectFactory::createInstance("independent", list));
      break;

    case Grid:
      list.setBoolean("jitter", false);
      sampler = static_cast<Sampler *>(
          ObjectFactory::createInstance("stratified", list));
      break;

    case Stratified:
      list.setBoolean("jitter", true);
      sampler = static_cast<Sampler *>(
          ObjectFactory::createInstance("stratified", list));
      break;
    }
  ...
  create_phong_bsdf(float exponent, float kd, float bsdfAngle) {
    PropertyList list;
    list.setFloat("exponent", exponent);
    list.setColor("kd", Color3f(kd));
    auto *brdf = (BSDF *)ObjectFactory::createInstance("phong", list);
  ...
  create_microfacet_bsdf(float alpha, float kd, float bsdfAngle) {
    ...
    list.setColor("kd", Color3f(kd));
    auto *brdf = (BSDF *)ObjectFactory::createInstance("microfacet", list);
...
public:
  WarpTestScreen()
      : Screen(nanogui::Vector2i(800, 600), "warptest: Sampling and Warping"),
        m_bRec(::Vector3f()) {
    inc_ref();
  ...
  static float mapParameter(WarpType warpType, float parameterValue) {
    if (warpType == Beckmann || warpType == PhongBRDF ||
  ...
  void refresh() {
    ...
    if (warpType == PhongBRDF) {
      ...
      std::tie(ptr, m_bRec) = WarpTest::create_phong_bsdf(
          100.f * parameterValue, parameter2Value, bsdfAngle);
      m_brdf.reset(ptr);
    }
    ...
    try {
      WarpTest tester(warpType, parameterValue, parameter2Value, m_brdf.get(),
                      m_bRec);
    ...
    if (!m_brdfValueCheckBox->checked() ||
        (warpType != PhongBRDF && warpType != MicrofacetBRDF))
      value_scale = 0.f;
    ...
    if (warpType != Square) {
      for (int i = 0; i < m_pointCount; ++i) {
        if (values(0, i) == 0.0f) {
          positions.col(i) =
              ::Vector3f::Constant(std::numeric_limits<float>::quiet_NaN());
          continue;
        }
        positions.col(i) =
  ...
  bool mouse_motion_event(const nanogui::Vector2i &p,
    ...
    if (!Screen::mouse_motion_event(p, rel, button, modifiers))
      m_arcball.motion(::Vector2i(p.x(), p.y()));
    return true;
  }
  ...
  bool mouse_button_event(const nanogui::Vector2i &p, int button, bool down,
    ...
    if (!Screen::mouse_button_event(p, button, down, modifiers)) {
      if (button == GLFW_MOUSE_BUTTON_1)
        m_arcball.button(::Vector2i(p.x(), p.y()), down);
    }
    return true;
  }
  ...
  void draw_contents() {
    ...
    view = Matrix4f::look_at(nanogui::Vector3f(0, 0, 4),
                             nanogui::Vector3f(0, 0, 0),
                             nanogui::Vector3f(0, 1, 0));
    ...
    model = Matrix4f::translate(nanogui::Vector3f(-0.5f, -0.5f, 0.0f)) * model;
    ...
    if (m_drawHistogram) {
      ...
      const int histHeight =
          (warpType == Square || warpType == Disk || warpType == Triangle)
              ? histWidth
              : histWidth / 2;
      const int verticalOffset = (height() - histHeight) / 2;

      drawHistogram(nanogui::Vector2i(spacer, verticalOffset),
                    nanogui::Vector2i(histWidth, histHeight),
                    m_textures[0].get());
      drawHistogram(nanogui::Vector2i(2 * spacer + histWidth, verticalOffset),
                    nanogui::Vector2i(histWidth, histHeight),
                    m_textures[1].get());
      ...
      if (m_warpTypeBox->selected_index() == MicrofacetBRDF ||
          m_warpTypeBox->selected_index() == PhongBRDF) {
  ...
  void drawHistogram(const nanogui::Vector2i &pos_,
    ...
    nanogui::Vector2f s =
        -(nanogui::Vector2f(pos_) + nanogui::Vector2f(0.25f, 0.25f)) /
        nanogui::Vector2f(size_);
    nanogui::Vector2f e =
        nanogui::Vector2f(size()) / nanogui::Vector2f(size_) + s;
  ...
  void initializeGUI() {
    ...
    m_window->set_position(nanogui::Vector2i(15, 15));
    m_window->set_layout(new GroupLayout());

    set_resize_callback([&](nanogui::Vector2i size) {
      m_arcball.setSize(::Vector2i(size.x(), size.y()));
    });

    m_arcball.setSize(::Vector2i(m_size.x(), m_size.y()));
    ...
    m_pointCountBox->set_fixed_size(nanogui::Vector2i(80, 25));
    ...
    m_warpTypeBox = new ComboBox(
        m_window,
        {"Square", "Triangle", "Disk", "Sphere", "Hemisphere (unif.)", "Hemisphere (cos)",
         "Beckmann distr.", "Phong BRDF", "Microfacet BRDF"});
    ...
    m_parameterBox->set_fixed_size(nanogui::Vector2i(80, 25));
    panel = new Widget(m_window);
    panel->set_layout(
        new BoxLayout(Orientation::Horizontal, Alignment::Middle, 0, 20));
    ...
    m_parameter2Box->set_fixed_size(nanogui::Vector2i(80, 25));
    ...
    m_angleBox->set_fixed_size(nanogui::Vector2i(80, 25));
    ...
    m_parameter2Slider->set_value(.5f);
...
std::tuple<WarpType, float, float> parse_arguments(int argc, char **argv) {
  ...
  return std::tuple<WarpType, float, float>{tp, value, value2};
}
...
int main(int argc, char **argv) {
  ...
  if (warpType == PhongBRDF) {
    ...
    BSDF *ptr;
    std::tie(ptr, bRec) =
        WarpTest::create_phong_bsdf(100.f * paramValue, param2Value, bsdfAngle);
    bsdf.reset(ptr);
